...
int find_gc_block(struct ssd_info *ssd, unsigned int channel, unsigned int chip, unsigned int die, unsigned int plane, int start_block, int page_type)
{
    int block = start_block;
    int active_page = ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[block].last_write_page;
    int next_page = active_page + 1;
    int cnt = 0;
    while (!(next_page != ssd->parameter->page_block && next_page % 3 == page_type) && cnt < 10)
    {
        block = (block + 1) % ssd->parameter->block_plane;
        active_page = ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[block].last_write_page;
        next_page = active_page + 1;
        cnt++;
    }
    if (cnt == 10)
    {
        block = (start_block + 1) % ssd->parameter->block_plane;
    }
    return block;
}
/**************************************************************************************
...
Status find_active_block(struct ssd_info *ssd, unsigned int channel, unsigned int chip, unsigned int die, unsigned int plane)
{
    ...
    int block_size_for_write = ssd->parameter->block_plane; // 为兼容GC优化设计，修改寻找活跃块的机制
...
Status copy_back(struct ssd_info *ssd, unsigned int channel, unsigned int chip, unsigned int die, struct sub_request *sub)
{
    ...
    if (ssd->parameter->greed_CB_ad == 1) /*允许贪婪使用copyback高级命令*/
    {
        ...
        if (ssd->dram->map->map_entry[sub->lpn].state != 0) /*说明这个逻辑页之前有写过，需要使用copyback+random input命令，否则直接写下去即可*/
        {
            ...
            {
                /*
                int tmp_ppn = ssd->dram->map->map_entry[sub->lpn].pn;
                int tmp_page_id = find_location(ssd,tmp_ppn);
                int tmp_tR = get_read_time(ssd,tmp_page_id);
                */
...
Status static_write(struct ssd_info *ssd, unsigned int channel, unsigned int chip, unsigned int die, struct sub_request *sub)
{
    ...
    if (ssd->dram->map->map_entry[sub->lpn].state != 0) //说明这个逻辑页之前有写过，需要使用先读出来，再写下去，否则直接写下去即可
    {
        ...
        {
            int old_ppn = ssd->dram->map->map_entry[sub->lpn].pn;
            t_R = get_read_time(ssd, old_ppn);
            //
    sub->next_state_predict_time=ssd->current_time+7*ssd->parameter->time_characteristics.tWC+ssd->parameter->time_characteristics.tR+(size((ssd->dram->map->map_entry[sub->lpn].state^sub->state)))*ssd->parameter->time_characteristics.tRC+(sub->size*ssd->parameter->subpage_capacity)*ssd->parameter->time_characteristics.tWC;
            sub->next_state_predict_time = ssd->current_time + 7 * ssd->parameter->time_characteristics.tWC + t_R + (size((ssd->dram->map->map_entry[sub->lpn].state ^ sub->state))) *
    ssd->parameter->time_characteristics.tRC + (sub->size * ssd->parameter->subpage_capacity) * ssd->parameter->time_characteristics.tWC; ssd->read_count++; ssd->update_read_count++;
        }
    }
    else
    {
    ...
    */
    ...
    ssd->channel_head[channel].current_time = ssd->current_time;
    ssd->channel_head[channel].next_state = CHANNEL_IDLE;
    ssd->channel_head[channel].next_state_predict_time = sub->next_state_predict_time - t_PROG;

    ssd->channel_head[channel].chip_head[chip].current_state = CHIP_WRITE_BUSY;
    ssd->channel_head[channel].chip_head[chip].current_time = ssd->current_time;
    ssd->channel_head[channel].chip_head[chip].next_state = CHIP_IDLE;
    // ssd->channel_head[channel].chip_head[chip].next_state_predict_time=time+ssd->parameter->time_characteristics.tPROG;
    ssd->channel_head[channel].chip_head[chip].next_state_predict_time = sub->next_state_predict_time;
    return SUCCESS;
}
...
struct ssd_info *process(struct ssd_info *ssd)
{
    ...
    random_num = rand() % ssd->parameter->channel_number; /*产生一个随机数，保证每次从不同的channel开始查询*/
    ...
    for (chan = 0; chan < ssd->parameter->channel_number; chan++)
    {
        ...
        if ((ssd->channel_head[i].current_state == CHANNEL_IDLE) || (ssd->channel_head[i].next_state == CHANNEL_IDLE && ssd->channel_head[i].next_state_predict_time <= ssd->current_time))
        {
            if (ssd->gc_request > 0) /*有gc操作，需要进行一定的判断*/
            {
                ...
                {
                    // get_read_sub_number_for_channel(ssd,i);
                    continue;
                }
            }
...
Status go_one_step(struct ssd_info *ssd, struct sub_request *sub1, struct sub_request *sub2, unsigned int aim_state, unsigned int command)
{
    ...
    if (command == NORMAL)
    {
        ...
        int page_id = location->page;
        t_R = get_read_time(ssd, page_id);
        t_PROG = get_prog_time(ssd, page_id);
        ...
        case SR_R_READ: {
            ...
            sub->next_state_predict_time = ssd->current_time + t_R;
            ...
            ssd->channel_head[location->channel].chip_head[location->chip].next_state_predict_time = ssd->current_time + t_R;
