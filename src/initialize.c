...
struct ssd_info *initiation(struct ssd_info *ssd)
{
    ...
    ssd->sumfile = fopen(ssd->sumfilename, "a+");
    if (ssd->sumfile == NULL)
    {
        printf("the sum file can't open\n");
        return NULL;
    }
    printf("\n");
...
struct parameter_value *load_parameters(char parameter_file[30])
{
    ...
    while (fgets(buf, 200, fp))
    {
        ...
        // 增加SLC MLC TLC选项参数，增加对MLC TLC的支持
        ...
        else if ((res_eql = strcmp(buf, "version")) == 0)
        {
            sscanf(buf + next_eql, "%d", &p->version);
        }
        else
        {
