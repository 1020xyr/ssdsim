...
struct ssd_info
{
    ...
    unsigned long gc_read_sub_sum_cnt;
    unsigned long gc_read_sub_zero_cnt;
    unsigned long queue_length_cnt;
    unsigned long avg_queue_length;
    unsigned long alloc_lsb_nums;
    unsigned long alloc_lsb_nums_for_prob;
    ...
    char statisticfilename2[100];
    char sumfilename[100]; //汇总数据
...
typedef struct buffer_group
{
    ...
    unsigned int stored;      // indicate the sector is stored in buffer or not. 1 indicates the sector is stored and 0 indicate the sector isn't stored.EX.  00110011 indicates the first, second, fifth,
                              // sixth sector is stored in buffer.
