...
struct ssd_info *simulate(struct ssd_info *ssd)
{
    ...
    while (1)
    {
        ...
        process(ssd);
        trace_output(ssd);
        if (res == 0 && ssd->request_queue == NULL)
        {
            break;
        }
    }
...
void trace_output(struct ssd_info *ssd)
{
    ...
    while (req != NULL)
    {
        printf_sub_request_for_debug(ssd, req);
        sub = req->subs;
        flag = 1;
        start_time = 0;
        end_time = 0;
...
void statistic_output(struct ssd_info *ssd)
{
    ...
    fprintf(ssd->outputfile, "                        include the flash write count leaded by "
                             "read requests\n");
    ...
    fprintf(ssd->statisticfile, "                        include the flash write count leaded by "
                                "read requests\n");
    ...
    fprintf(ssd->sumfile, "                        include the flash write count leaded by "
                          "read requests\n");
